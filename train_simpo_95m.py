#!/usr/bin/env python3
"""
SimPO Training with 95M Parameter Model
Runs on CPU — trains for 5 hours by default.

Uses the autoresearch GPT architecture (95M params):
...
def train_simpo_95m():
    """Train 95M model with SimPO for 5 hours."""
    print("=" * 60)
    print("SimPO Training — 95M Parameter Model")
    print("=" * 60)
    ...
    config = GPT95MConfig(sequence_len=max_len)
    ...
    step = 0
    total_loss = 0
    os.makedirs("./simpo-95m", exist_ok=True)
    ...
    while True:
        ...
        for batch in loader:
            ...
            if step % (save_every // 2) == 0:  # roughly every 10 min
                path = f"./simpo-95m/step_{step}.pt"
                torch.save(
                    {
                        "step": step,
